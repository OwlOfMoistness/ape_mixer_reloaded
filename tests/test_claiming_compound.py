...
def test_claiming_from_bayc(matcher, ape, bayc, smooth, nft_guy, dog_guy, coin_guy, chain, compounder, ape_staking, admin):
	...
	assert math.isclose(matcher.payments(compounder), pending // 2 * 96 // 100)
	...
	assert math.isclose(compounder.liquid() - pre_coin - pool0_rewards, (pending + pending_2) * 96 // 100 // 2)
	assert math.isclose(ape.balanceOf(smooth) - matcher.fee() - pre_smooth, (pending + pending_2) * 96 // 100 // 2 )
	matcher.batchClaimRewardsFromMatches([0], 2, {'from':nft_guy})
	assert math.isclose(matcher.payments(nft_guy), 0)
	assert compounder.balanceOf(nft_guy) > 0
	assert math.isclose(compounder.balanceOf(nft_guy) * compounder.pricePerShare() // 1e18, (pending + pending_2) * 96 // 100 // 2)
